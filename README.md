...
- 上下文跟随：`/game switch` 后**角色卡 / 暗骰子区 / 房规**三项自动切换，解析顺序"局 > 当前场景 > 父频道 > **用户级常用卡** > 全局"；`/pc tag` 会同时把卡记成**用户级常用卡**（按服务器），所以没单独设过的频道/子区都继续用它（跨子区持续）；**日志不跟随局**——日志的作用域是"开 log 的那个场景"（单个频道或单个子区），不同频道/子区可以同时各开一条
...
- `/log`：作用域是**当前场景**；`/log new` 只在本场景已有 `on` 日志时**拒绝**，其他频道/子区不受影响，且**不再自动建局**（当前场景没有局时只开场景日志；要开团须显式 `/game start`，或 `/log new game:<桌名|#N>` 挂到已有局）；`/log`（无参）默认记录**开 log 的那个场景**的发言，`/log end` 只结束本场景或指名的日志、在记录中不自动结束已有日志；`/log list`、`/log end [name]`
